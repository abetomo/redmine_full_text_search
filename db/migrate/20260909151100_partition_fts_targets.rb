...
class PartitionFtsTargets < ActiveRecord::Migration[6.1]
  ...
  def up
    ...
    # Don't stop the migration because this plugin works without partitioning.
    # Run "full_text_search:partition:up" after upgrading Groonga and PGroonga.
    unless FullTextSearch::Partition.available?
      say "Skipped: #{FullTextSearch::Partition.requirements_message}"
      return
    end
...
CREATE UNIQUE INDEX index_fts_targets_on_id_and_registered_at
  ...
  def down
    ...
    # "up" is skipped when Groonga or PGroonga is old.
    return unless FullTextSearch::Partition.partitioned?
